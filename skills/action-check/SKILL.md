...
- **If they're missing**, tell the user one line — "GitHub MCP actions toolset isn't registered; running `/setup-mcp github` first." — then invoke the `setup-mcp` skill via the Skill tool with argument `github`. Its `mcps.json` entry enables the actions toolset via the `X-MCP-Toolsets: all` header. After it completes, the user must restart Claude Code; stop the current run and ask them to re-invoke `/action-check` once the new server loads.
