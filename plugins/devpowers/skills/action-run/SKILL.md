...
```bash
${CLAUDE_PLUGIN_ROOT}/skills/action-run/run.sh <workflow-file> [key=value|flag ...]
```

Examples:
```bash
${CLAUDE_PLUGIN_ROOT}/skills/action-run/run.sh publish.yaml tag=v0.0.8 pre-release
${CLAUDE_PLUGIN_ROOT}/skills/action-run/run.sh generate-migration.yaml message="add users table"
${CLAUDE_PLUGIN_ROOT}/skills/action-run/run.sh test.yaml
```
